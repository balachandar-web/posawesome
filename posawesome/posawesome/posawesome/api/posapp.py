...
def submit_invoice(invoice, data):
    ...
    frappe.logger().info(f"POS Invoice {invoice_doc.name} attempting submission")
    
    # Check if background job submission is allowed
    use_background_job = frappe.get_value(
        "POS Profile",
        invoice_doc.pos_profile,
        "posa_allow_submissions_in_background_job",
    )
    
    # Handle case where background job is preferred
    if use_background_job:
        try:
            frappe.logger().info(f"Attempting background submission for invoice {invoice_doc.name}")
            ...
            # Log the number of pending invoices
            frappe.logger().info(f"Found {len(invoices_list)} pending invoices for background submission")
            
            # Queue each invoice for submission
            for invoice in invoices_list:
                frappe.logger().info(f"Enqueueing invoice {invoice.name} for background submission")
                enqueue(
            ...
            # Force immediate submission of current invoice if it's not in the list
            current_in_list = any(inv.name == invoice_doc.name for inv in invoices_list)
            if not current_in_list:
                frappe.logger().info(f"Current invoice {invoice_doc.name} not in pending list, submitting directly")
                invoice_doc.submit()
                redeeming_customer_credit(
                    invoice_doc, data, is_payment_entry, total_cash, cash_account, payments
                )
        except Exception as e:
            # If background submission fails, fall back to direct submission
            frappe.logger().error(f"Background submission failed for {invoice_doc.name}: {str(e)}")
            frappe.logger().info(f"Falling back to direct submission for invoice {invoice_doc.name}")
            invoice_doc.submit()
            redeeming_customer_credit(
                invoice_doc, data, is_payment_entry, total_cash, cash_account, payments
            )
    else:
        # Direct submission as per original logic
        frappe.logger().info(f"Directly submitting invoice {invoice_doc.name}")
        invoice_doc.submit()
        redeeming_customer_credit(
            invoice_doc, data, is_payment_entry, total_cash, cash_account, payments
        )
...
def submit_in_background_job(kwargs):
    ...
    try:
        ...
        frappe.logger().info(f"Background job: Starting submission for invoice {invoice}")
        
        # Check if invoice exists and is in draft state
        invoice_doc = frappe.get_doc("Sales Invoice", invoice)
        if invoice_doc.docstatus != 0:
            frappe.logger().info(f"Background job: Invoice {invoice} already processed (docstatus={invoice_doc.docstatus}). Skipping.")
            return
            
        # Force reload to make sure we have the latest version
        invoice_doc.reload()
        
        # Submit the invoice
        frappe.logger().info(f"Background job: Submitting invoice {invoice}")
        invoice_doc.submit()
        frappe.db.commit()
        
        # Handle customer credit after successful submission
        frappe.logger().info(f"Background job: Processing customer credit for invoice {invoice}")
        redeeming_customer_credit(
            invoice_doc, data, is_payment_entry, total_cash, cash_account, payments
        )
        frappe.db.commit()
        
        # Final log entry for successful submission
        frappe.logger().info(f"Background job: Successfully completed submission for invoice {invoice}")
        
    except Exception as e:
        frappe.db.rollback()
        frappe.logger().error(f"Background job: Error submitting invoice {invoice}: {str(e)}")
        
        # Check if invoice still exists
        try:
            invoice_doc = frappe.get_doc("Sales Invoice", invoice)
            if invoice_doc.docstatus == 0:  # Still in draft
                frappe.logger().info(f"Background job: Attempting retry submission for invoice {invoice}")
                invoice_doc.submit()
                frappe.db.commit()
                frappe.logger().info(f"Background job: Retry successful for invoice {invoice}")
            else:
                frappe.logger().info(f"Background job: Invoice {invoice} status changed to {invoice_doc.docstatus}. No retry needed.")
        except Exception as retry_error:
            frappe.db.rollback()
            frappe.logger().error(f"Background job: Retry failed for invoice {invoice}: {str(retry_error)}")
