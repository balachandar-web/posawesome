    ...
    <v-card style="max-height: 70vh; height: 70vh" class="cards my-0 py-0 mt-3 bg-grey-lighten-5">
      ...
      <div class="my-0 py-0 overflow-y-auto" style="max-height: 60vh">
        <v-data-table :headers="items_headers" :items="items" v-model:expanded="expanded" show-expand
          ...
          hide-default-footer>
          <!-- Force display of all columns by setting fixed widths -->
          <template v-slot:headers="{ headers }">
            <tr>
              <th v-for="header in headers" :key="header.key" :style="{
                minWidth: header.key === 'item_name' ? '30%' :
                  header.key === 'qty' ? '15%' :
                  header.key === 'stock_qty' ? '12%' :
                  header.key === 'uom' ? '10%' :
                  header.key === 'rate' ? '15%' :
                  header.key === 'amount' ? '15%' : '5%',
                whiteSpace: 'nowrap',
                overflow: 'visible',
              ...
              <th style="width: 36px"></th> <!-- Expand column -->
            </tr>
          </template>
          <template v-slot:item="{ item, index, columns }">
            <tr>
              <td>{{ item.item_name }}</td>
              <td style="text-align: center; font-weight: bold">{{ formatFloat(item.qty || 0) }}</td>
              <td style="text-align: center; color: #2E7D32">{{ formatFloat(item.stock_qty || 0) }}</td>
              <td style="text-align: center">{{ item.uom }}</td>
              <td style="text-align: center">
                {{ currencySymbol(pos_profile.currency) }} {{ formatCurrency(item.rate) }}
              </td>
              <td style="text-align: center">
                {{ currencySymbol(pos_profile.currency) }} {{ formatCurrency(flt(item.qty, float_precision) * flt(item.rate, currency_precision)) }}
              </td>
              <td style="text-align: center">
                <v-checkbox-btn :model-value="!!item.posa_is_offer || !!item.posa_is_replace" class="center" disabled></v-checkbox-btn>
              </td>
              <td>
                <v-btn icon="mdi-chevron-down" variant="text" density="comfortable" @click="expanded = expanded.includes(item.posa_row_id) ? expanded.filter(id => id !== item.posa_row_id) : [...expanded, item.posa_row_id]"></v-btn>
              </td>
            </tr>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      items_headers: [
        ...
        { title: __("Available QTY"), key: "stock_qty", align: "center" },
  ...
  methods: {
    ...
    get_invoice_items() {
      ...
      if (!this.items || !Array.isArray(this.items) || this.items.length === 0) {
        console.log('Invoice.vue: No items available for invoice');
        return items_list;
      }
    ...
    get_payments() {
      const payments = [];
      // Check if pos_profile and payments array exist
      if (!this.pos_profile || !this.pos_profile.payments || !Array.isArray(this.pos_profile.payments)) {
        console.log('Invoice.vue: No payment methods available in pos_profile');
        return payments;
      }
    ...
    async show_payment(showPaymentFlag) {
      console.log('Invoice.vue: show_payment method called with value:', showPaymentFlag);
      
      // Clear any stale processing flags that might be stuck
      if (this._processingPayment) {
        console.log('Invoice.vue: Clearing stale processing flag');
        this._processingPayment = false;
      }
      
      // Handle different types of input (including PointerEvent from button clicks)
      let isShowingPayment = true; // Default to showing payment
      
      if (showPaymentFlag !== undefined) {
        if (typeof showPaymentFlag === 'boolean') {
          isShowingPayment = showPaymentFlag;
        } else if (typeof showPaymentFlag === 'string') {
          isShowingPayment = showPaymentFlag === 'true';
        } else if (typeof showPaymentFlag === 'object' && showPaymentFlag !== null) {
          console.log('Invoice.vue: Received click event, treating as true');
          isShowingPayment = true;
        }
      }
      
      console.log('Invoice.vue: Interpreted as:', isShowingPayment ? 'show payment screen' : 'hide payment screen');
      
      // If we're hiding the payment screen, handle it directly
      if (!isShowingPayment) {
        console.log('Invoice.vue: Hiding payment screen');
        this._processingPayment = false;
        this._lastPaymentScreenState = false;
        this.eventBus.emit("show_payment", 'false');
        return true;
      }
      
      // Use a timeout-based recursion protection instead of a simple flag
      // This prevents the flag from getting stuck if an error occurs
      const currentTimestamp = Date.now();
      if (this._lastProcessingTime && (currentTimestamp - this._lastProcessingTime < 1000)) {
        console.warn('Invoice.vue: Payment processed too recently, preventing potential recursive call');
        return false;
      }
      
      // Start new payment processing
      this._lastProcessingTime = currentTimestamp;
      this._processingPayment = true;
      this._lastPaymentScreenState = false; // Reset state before starting
      
      // Listen for get_invoice_doc event to provide latest invoice data
      this.eventBus.on('get_invoice_doc', () => {
        console.log('Invoice.vue: Received request for invoice doc');
        if (this.invoice_doc) {
          console.log('Invoice.vue: Sending invoice doc to payment screen');
          this.eventBus.emit('send_invoice_doc_payment', this.invoice_doc);
        } else {
          console.warn('Invoice.vue: No invoice doc available');
        }
      });
      
      try {
        // Check if we have a customer selected
        let customerSelected = this.customer;
        
        // Try to get customer from store if not available in component
        if (!customerSelected && customerStore && customerStore.state) {
          console.log('Invoice.vue: Attempting to get customer from store');
          customerSelected = customerStore.state.selectedCustomer;
          
          if (customerSelected) {
            console.log('Invoice.vue: Retrieved customer from store:', customerSelected);
        ...
        if (!customerSelected) {
          console.warn('Invoice.vue: No customer selected when attempting to show payment');
          this.eventBus.emit("show_message", {
        ...
        if (!this.items || !this.items.length) {
          console.warn('Invoice.vue: No items selected when attempting to show payment');
          this.eventBus.emit("show_message", {
        ...
        if (!validationResult) {
          console.warn('Invoice.vue: Validation failed');
          this._processingPayment = false;
          return false;
        }
        
        // Log that we're proceeding with payment after validation
        console.log('Invoice.vue: Validation passed, proceeding with payment process');
      } catch (error) {
        console.error('Invoice.vue: Error during payment validation:', error);
        this.eventBus.emit("show_message", {
          title: __(`Error validating payment: ${error.message || 'Unknown error'}`),
          color: "error",
        });
        return false;
      }
      
      // Ensure customer is properly synchronized with the store
      if (customerStore && typeof customerStore.dispatch === 'function' && this.customer) {
        console.log('Invoice.vue: Syncing customer with store before payment:', this.customer);
        customerStore.dispatch('selectCustomer', this.customer);
      }
      
      // Show the payment screen and send initial invoice data
      console.log('Invoice.vue: Emitting show_payment event with true');
      this.eventBus.emit("show_payment", 'true');
      
      // Add a slight delay then force payment screen visibility directly
      setTimeout(() => {
        console.log('Invoice.vue: Forcing payment screen visibility');
        this.eventBus.emit('force_payment_screen_visibility', true);
      }, 100);
      
      if (this.invoice_doc) {
        console.log('Invoice.vue: Sending initial invoice doc to payment screen');
        this.eventBus.emit('send_invoice_doc_payment', this.invoice_doc);
        
        // Send invoice doc again after slight delay for redundancy
        setTimeout(() => {
          console.log('Invoice.vue: Sending invoice doc again for redundancy');
          this.eventBus.emit('send_invoice_doc_payment', this.invoice_doc);
        }, 300);
      }
      
      // Update state after successful validation and event emission
      this._lastPaymentScreenState = true;
      this._processingPayment = false;
      
      // Set a timer to reset the processing timestamp after 1 second
      // This allows a new payment attempt after a short cooldown period
      setTimeout(() => {
        this._lastProcessingTime = null;
        console.log('Invoice.vue: Payment processing cooldown completed');
      }, 1000);
      
      return true; // Indicate successful payment initiation
      
      // Initialize invoice_doc if it doesn't exist
      if (!this.invoice_doc) {
        console.log('Invoice.vue: Initializing new invoice document');
        // Create a basic invoice document
        this.invoice_doc = {
      ...
      } else if (typeof this.invoice_doc === 'string') {
        // Convert string to object if needed
        console.log('Invoice.vue: Converting invoice_doc from string to object');
        this.invoice_doc = {
          doctype: "Sales Invoice",
          is_pos: 1,
          ignore_pricing_rule: 1,
      ...
      // Double-check that invoice_doc is now available
      if (!this.invoice_doc || typeof this.invoice_doc !== 'object') {
        console.error('Invoice.vue: Failed to initialize invoice document');
        this.eventBus.emit("show_message", {
          title: __(`Error: Could not create invoice document`),
          color: "error",
        });
        return false;
      }
      try {
        if (this.invoice_doc.doctype == "Sales Order") {
          console.log('Invoice.vue: Processing Sales Order payment');
          
          // Use a consistent event name with string parameter
          console.log('Invoice.vue: Emitting show_payment=true event');
          this.eventBus.emit("show_payment", 'true');
          
          // Then process the invoice
          const invoice_doc = await this.process_invoice_from_order();
          
          // Only emit the send_invoice_doc_payment if we have a valid invoice_doc
          if (invoice_doc) {
            console.log('Invoice.vue: Sending invoice doc to payment component');
            this.eventBus.emit("send_invoice_doc_payment", invoice_doc);
          } else {
            console.error('Invoice.vue: Failed to process invoice from order');
            this.eventBus.emit("show_message", {
          ...
          // Make sure we have the required parameters for the API call
          if (!this.invoice_doc.items || !this.invoice_doc.items.length || !this.invoice_doc.items[0] || !this.invoice_doc.items[0].name) {
            console.warn('Invoice.vue: Missing required invoice item data for get_sales_invoice_child_table');
            // Create an empty sales_invoice_item_doc to continue processing
            var sales_invoice_item_doc = {};
          } else {
            const sales_invoice_item = this.invoice_doc.items[0];
            var sales_invoice_item_doc = {};
            
            // Only make the API call if we have both required parameters
            if (this.invoice_doc.name && sales_invoice_item.name) {
              ...
              console.warn('Invoice.vue: Missing required parameters for get_sales_invoice_child_table');
            }
          }
          if (sales_invoice_item_doc.sales_order) {
            // Use consistent show_payment event with string parameter
            console.log('Invoice.vue: Emitting show_payment=true event');
            this.eventBus.emit("show_payment", 'true');
            const invoice_doc = await this.process_invoice_from_order();
            this.eventBus.emit("send_invoice_doc_payment", invoice_doc);
          } else {
            // Directly emit show_payment with a string value to ensure consistent handling
            console.log('Invoice.vue: Directly emitting show_payment=true event');
            this.eventBus.emit("show_payment", 'true');
            
            // Process the invoice and send it to the payment component
        ...
        console.error('Invoice.vue: Error in show_payment method:', error);
        this.eventBus.emit("show_message", {
          title: __(`Error showing payment screen: ${error.message || 'Unknown error'}`),
          color: "error",
        });
      } finally {
        // Reset payment processing flag regardless of success or failure
        console.log('Invoice.vue: Resetting payment processing flag');
        this._processingPayment = true;
      }
    ...
    validate() {
      // Add a guard against recursive validation calls
      if (this._validating) {
        console.warn('Invoice.vue: Preventing recursive validation call');
        return false;
      }
      ...
      try {
        ...
        if (!this.customer) {
          console.warn('Invoice.vue: No customer selected during validation');
          this.eventBus.emit("show_message", {
            title: __("Select a customer"),
            color: "error",
          });
        ...
        console.log('Invoice.vue: Validation completed with result:', value);
        return value;
      } catch (error) {
        console.error('Invoice.vue: Error in validate method:', error);
        return false;
      } finally {
        // Reset validation flag regardless of success or failure
    ...
    // close_payments() {
    //   console.log('Invoice.vue: Calling show_payment method with false value');
    //   // Directly call the show_payment method with false parameter
    ...
    calc_stock_qty(item, value) {
      // Make sure we have valid values for calculation
      const conversion_factor = parseFloat(item.conversion_factor || 1);
      const qty = parseFloat(value || 0);
      
      // Calculate stock_qty and ensure it's a valid number
      item.stock_qty = conversion_factor * qty;
      
      // If stock_qty is NaN or undefined, set a default value of 0
      if (isNaN(item.stock_qty) || item.stock_qty === undefined) {
        item.stock_qty = 0;
      }
  ...
  beforeUnmount() {
    ...
    if (eventBus) {
      ...
      eventBus.off("add_item");
      eventBus.off("update_customer");
      eventBus.off("fetch_customer_details");
      eventBus.off("clear_invoice");
      
      // Payment screen events
      eventBus.off("get_invoice_doc");
      
      // Offer related events
      eventBus.off("set_offers");
      eventBus.off("update_invoice_offers");
      eventBus.off("update_invoice_coupons");
      eventBus.off("set_all_items");
      
      console.log('Invoice.vue: Cleaned up all event listeners');
    }
  },
  created() {
