...
export default {
  ...
  methods: {
    back_to_invoice() {
      console.log('Payments.vue: Closing payment screen');
      ...
      this.$nextTick(() => {
        this.$forceUpdate();
        console.log('Payments.vue: Payment screen closed');
      });
    },
    ...
    submit_invoice(print) {
      console.log('Payments.vue: Starting invoice submission process');
      ...
      // Update POS flag based on return status
      if (this.invoice_doc.is_return && totalPayedAmount == 0) {
        this.invoice_doc.is_pos = 0;
      }
      
      // Process customer credit information
      ...
      console.log('Payments.vue: Prepared data for submission', { 
      ...
      // Show user a processing message
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (!r?.message) {
            console.error('Payments.vue: Error submitting invoice - no response message');
            vm.eventBus.emit("show_message", {
          ...
          console.log('Payments.vue: Invoice submission response', { 
          ...
          if (r.message.status !== 1) {
            console.warn('Payments.vue: Invoice not fully submitted (status=' + r.message.status + '). Verifying status...');
            ...
            setTimeout(() => {
              frappe.call({
                ...
                callback: function(invoice_status) {
                  if (invoice_status.message && invoice_status.message.docstatus === 1) {
                    console.log('Payments.vue: Invoice verified as submitted after double-check');
                    vm.completeSuccessfulSubmission(r.message.name, print);
                  } else {
                    console.error('Payments.vue: Invoice still not submitted after verification');
                    vm.eventBus.emit("show_message", {
                      title: `Warning: Invoice may not be fully submitted`,
                      color: "warning",
                    });
                    vm.completeSuccessfulSubmission(r.message.name, print);
                  }
            ...
            }, 2000); // Wait 2 seconds for background job to complete
          } else {
            // Proceed with normal completion for successfully submitted invoice
            vm.completeSuccessfulSubmission(r.message.name, print);
          }
        }
      });
    ...
    // New method to handle successful submission
    completeSuccessfulSubmission(invoice_name, print) {
      console.log('Payments.vue: Completing successful submission process for', invoice_name);
      ...
      // Update UI
      this.eventBus.emit("set_last_invoice", invoice_name);
      this.eventBus.emit("show_message", {
        title: `Invoice ${invoice_name} is Submitted`,
        color: "success",
      });
      
      // Play sound and clear invoice
  ...
  mounted: function () {
    this.$nextTick(function () {
      console.log('Payments.vue: Component mounted', new Date().toISOString());
      
      // Initialize component visibility
      this.loading = false;
      this.invoice_doc = this.invoice_doc || {};
      
      // Clear Paid Amount and To Be Paid fields
      this.invoice_doc.payments.forEach(payment => {
        payment.amount = 0;
      });
      ...
      // Immediately request invoice data when component is mounted
      console.log('Payments.vue: Automatically requesting invoice doc on mount');
      setTimeout(() => {
        this.eventBus.emit("get_invoice_doc");
      }, 50);
      
      // Listen for payment screen preparation
      this.eventBus.on("prepare_payment_screen", (show) => {
        if (show) {
          console.log('Payments.vue: Preparing payment screen', new Date().toISOString());
          // Ensure we have the latest invoice doc
          this.eventBus.emit("get_invoice_doc");
          ...
          // Make sure component is properly initialized and visible
          setTimeout(() => {
            // Force component update
            this.$forceUpdate();
            
            // Force parent containers to show this component
            const parentElement = this.$el.parentElement;
            if (parentElement) {
              console.log('Payments.vue: Setting parent element to display:block');
              parentElement.style.display = 'block';
            }
            
            console.log('Payments.vue: Payment screen should be visible now');
            
            // Request invoice data again for redundancy
            this.eventBus.emit("get_invoice_doc");
          }, 100);
        }
      });
      
      // Add a direct force visibility event handler
      this.eventBus.on('force_payment_screen_visibility', () => {
        console.log('Payments.vue: Received force_payment_screen_visibility event');
        setTimeout(() => {
          // Force component update
          this.$forceUpdate();
          
          // Request latest invoice data
          this.eventBus.emit("get_invoice_doc");
        }, 50);
      });

      this.eventBus.on("send_invoice_doc_payment", (invoice_doc) => {
        console.log('Payments.vue: Received invoice doc for payment');
        ...
        // Force component update after data is ready
        this.$forceUpdate();
        console.log('Payments.vue: Payment screen data initialized');
      });
    ...
    this.eventBus.on("update_customer", (customer) => {
      if (this.customer != customer) {
        ...
        if (customerStore && typeof customerStore.dispatch === 'function') {
          console.log('Payments.vue: Syncing with customer store');
    ...
    // Listen for customer store changes
    if (customerStore && customerStore.state) {
      if (customerStore.state.selectedCustomer && !this.customer) {
        this.customer = customerStore.state.selectedCustomer;
      }
    }
